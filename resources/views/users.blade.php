                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <h2 class="text-primary">{{$users_list->count()}}</h2>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <h2 class="text-success">{{$users_list->where('is_banned', false)->count()}}</h2>
                            </div>
                            <div class="align-self-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor"
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <h2 class="text-danger">{{$users_list->where('is_banned', true)->count()}}</h2>
                            </div>
                            <div class="align-self-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor"
                        ...
                        @foreach($users_list as $user)
                            @if($user->role !== "admin")
                                <tr class="user-status-active">
                                    ...
                                    <td>{{$user->nickname}}</td>
                                    <td>{{$user->email}}</td>
                                    <td>{{Carbon::make($user->created_at)->format('Y-m-d')}}</td>
                                    <td>{{$user->posts()->count()}}</td>
                                    <td>
                                        @if(!$user->is_banned)
                                            <span class="badge badge-active bg-success">Не забанен</span>
                                        @else
                                            <span class="badge badge-active bg-danger">Забанен</span>
                                        @endif
                                    </td>
                                    <td>
                                        <form method="post" action="/admin/users/{{$user->id}}">
                                            ...
                                            @method('PATCH')
                                            @if(!$user->is_banned)
                                                <button class="btn btn-danger btn-sm">Заблокировать</button>
                                            @else
